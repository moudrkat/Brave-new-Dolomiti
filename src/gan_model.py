...
def build_generator(latent_dim=100, width=64, drop=0.4):
    ...
    model.add(tf.keras.layers.Conv2D(filters = 64, kernel_size = 4, padding = 'same'))
    model.add(tf.keras.layers.BatchNormalization(momentum = 0.7))
    model.add(tf.keras.layers.Activation('relu'))
    model.add(tf.keras.layers.UpSampling2D())
    assert model.output_shape == (None, 32, 32, 64)
    
    model.add(tf.keras.layers.Conv2D(filters = 32, kernel_size = 4, padding = 'same'))
    model.add(tf.keras.layers.BatchNormalization(momentum = 0.7))
    model.add(tf.keras.layers.Activation('relu'))
    model.add(tf.keras.layers.UpSampling2D())
    assert model.output_shape == (None, 64, 64, 32)
    
    model.add(tf.keras.layers.Conv2D(filters = 16, kernel_size = 4, padding = 'same'))
    model.add(tf.keras.layers.BatchNormalization(momentum = 0.7))
    model.add(tf.keras.layers.Activation('relu'))
    model.add(tf.keras.layers.UpSampling2D())
    assert model.output_shape == (None, 128, 128, 16)
    
    model.add(tf.keras.layers.Conv2D(filters = 8, kernel_size = 4, padding = 'same'))
    model.add(tf.keras.layers.Activation('relu'))
    model.add(tf.keras.layers.UpSampling2D())
...
def build_discriminator(img_width=256, img_height=256, width=64, p=0.4):
    model = tf.keras.Sequential()

    #add Gaussian noise to prevent Discriminator overfitting
    model.add(tf.keras.layers.GaussianNoise(0.2, input_shape = [256, 256, 3]))
    
    # First convolutional layer
    model.add(tf.keras.layers.Conv2D(width*1, kernel_size=4, strides=2, padding='same', input_shape=(img_width, img_height, 3)))  # RGB input
    model.add(tf.keras.layers.LeakyReLU(0.2))
    model.add(tf.keras.layers.Dropout(p))
    
    # Second convolutional layer
    model.add(tf.keras.layers.Conv2D(width*2, kernel_size=4, strides=2, padding='same'))
    model.add(tf.keras.layers.LeakyReLU(0.2))
    model.add(tf.keras.layers.Dropout(p))
    
    # Third convolutional layer
    model.add(tf.keras.layers.Conv2D(width*4, kernel_size=4, strides=2, padding='same'))
    model.add(tf.keras.layers.LeakyReLU(0.2))
    model.add(tf.keras.layers.Dropout(p))
    
    # Fourth convolutional layer
    model.add(tf.keras.layers.Conv2D(width*8, kernel_size=4, strides=2, padding='same'))
    model.add(tf.keras.layers.LeakyReLU(0.2))
    model.add(tf.keras.layers.Dropout(p))
    
    # Fifth convolutional layer (to further downscale)
    model.add(tf.keras.layers.Conv2D(width*16, kernel_size=4, strides=2, padding='same'))
    model.add(tf.keras.layers.LeakyReLU(0.2))
    model.add(tf.keras.layers.Dropout(p))
...
def train_gan(sketch_type,generator, discriminator, gan, images, image_placeholder,image_placeholder_loss, epochs=100, batch_size=64, latent_dim=100):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1 == 0:  # Save model and show images every 100 epochs
            ...
            show_images_in_streamlit(denormalized_real_images, denormalized_fake_images, epoch, image_placeholder)  # Show images in Streamlit
